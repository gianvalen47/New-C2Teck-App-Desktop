...
const columns = ["Cód.", "Nombre", "Rubro", "Moneda", "Precio Oferta", "Observación", "Fec. Venc.", "Estado"];
  ...
  return (
    <div className="relative h-full min-h-0 flex flex-col overflow-hidden bg-[#F3F6FA]">
      ...
      <div className="flex flex-wrap items-end gap-2 px-2 py-1.5 bg-[#F0F4F8] border-b border-slate-300">
        <Field label="Estado" className="w-40"><select className={inp} defaultValue="(Todos)"><option>(Todos)</option><option>Vigente</option><option>Expirado</option><option>Programado</option></select></Field>
        <Field label="Rubro" className="w-44"><select className={inp} defaultValue="(Todos)"><option>(Todos)</option><option>REPUESTOS</option><option>SERVICIOS</option><option>LUBRICANTES</option></select></Field>
