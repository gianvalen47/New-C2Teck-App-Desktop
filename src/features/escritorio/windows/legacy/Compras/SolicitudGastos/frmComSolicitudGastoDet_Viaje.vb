...
Imports System.IO
Imports System.Xml
Public Class frmComSolicitudGastoDet_Viaje
    ...
    Private Sub frmComSolicitudGastoDet_Viaje_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        If state_button Then                'Modificar
            ...
            'End If
        Else                                      'Nuevo
            Me.Size = New System.Drawing.Size(675, 430)
    ...
    Private Sub frmComSolicitudGastoDet_Viaje_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub enableOpciones()
        ...
        Else
            ...
            dgvJos.RootTable.Columns("ProcesoJob").Visible = IIf((Session.CodPerfil = "01" Or Session.CodPerfil = "24") And (estado = 3 Or estado = 5 Or estado = 7), True, False)
            dgvJos.RootTable.Columns("CodJob").Width = IIf((Session.CodPerfil = "01" Or Session.CodPerfil = "24") And (estado = 3 Or estado = 5 Or estado = 7), "67", "88")
        End If

        miAsignarCentroCosto.Enabled = IIf(Not edicion And editable And Planilla = False And GastoViaje = False And (estado = 1 Or ((estado = 3 Or estado = 5) And (Session.CodPerfil = "22" Or Session.CodPerfil = "01"))), True, False)
        miAsignarJobs.Enabled = IIf(Not edicion And editable And Planilla = False And GastoViaje = False And (estado = 1 Or ((estado = 3 Or estado = 5) And (Session.CodPerfil = "22" Or Session.CodPerfil = "01"))), True, False)
        ...
        Try
            ...
            If state_button Then    '////////////////////////////////////////////////// ACTUALIZAR ////////////////////////////////////////////////////////////
                ...
                Else      '=========================== <> GENERADO==========================
                    ...
                    If (estado = 2 Or estado = 3 Or (estado = 5 And (Session.CodPerfil = "22" Or Session.CodPerfil = "01"))) Then 'And Planilla = False Then 'ESTADO( APROBADO POR JEFE DE AREA O APROBADO) Se activa los detalles tipo Planilla para la edición de Observación Contable (Sr. Jesus Alba 03/08/2015)
                        cbAfectoIgv.Enabled = True
                        btnAgregarProveedor.Enabled = True
                        btnBuscarProveedor.Enabled = True
                        btnLimpiarProveedor.Enabled = True  'Agregado el 20/09/2012
                        ...
                        If Session.CodPerfil = "22" Or Session.CodPerfil = "01" Then
                            txtSubTotal.ReadOnly = False
                            txtSubTotal.BackColor = System.Drawing.SystemColors.Window
                            txtMontoNoAfecto.ReadOnly = False
                            txtMontoNoAfecto.BackColor = System.Drawing.SystemColors.Window
    ...
    Private Function ValidaCampos() As Boolean
        Try
            ...
            ElseIf txtIgv.Value < 0 Then
                ...
                txtIgv.Focus()
                Return False
            ElseIf toDouble(txtSubTotal.Value) <= 0 And cbAfectoIgv.Checked = True Then    'toDouble(txtMonto.Value) <= 0 Then  ' Modificado 26-02
    ...
    Private Sub ObtenerRegistro()
        Try
            ...
            Else
                cmbTipoDoc.SelectedIndex = 0
            End If
            txtSubTotal.Value = registro.Monto
    ...
    Private Sub NuevoCentroCosto()
        Try
            ...
            registro = oSolicitudGastoDetService.Obtener(IdGastoDet)

            frm.IdGasto = IdGasto
            frm.Monto = registro.Monto
            frm.MontoSinIgv = registro.MontoSinIgv
        ...
        Try
            If MsgBox("¿Está seguro de GUARDAR los datos?", MsgBoxStyle.YesNo, "Advertencia") = MsgBoxResult.Yes Then
                If ValidaCampos() Then
                    ...
                    Dim registroanterior As New SolicitudGastoDetService.SolicitudGastoDet

                    If state_button = True Then
                        registroanterior = oSolicitudGastoDetService.Obtener(utils.toNumber(IdGastoDet))                
                    End If
                    ...
                    Rubro.CodRubro = Nothing
                    registro.RubroGasto = Rubro
                    TipoGasto.IdTipoGasto = Nothing
                    ...
                    CondicionPago.IdCondicion = IIf(utils.toNumber(IdProveedor) <> 0, 1, Nothing)
                    ...
                    registro.Monto = utils.toDouble(txtSubTotal.Value + txtMontoIgv.Value)
                    registro.MontoSinIgv = utils.toDouble(txtSubTotal.Value + txtMontoNoAfecto.Value)
                    ...
                    registro.Justificacion = IIf(state_button = True, registroanterior.Justificacion, Nothing)
    ...
    Private Sub txtProveedor_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtProveedor.TextChanged
        Try
            If txtProveedor.Text <> "" Then
                ...
                '    cmbTipoDoc.Value = proveedor.CondicionPagoProveedor.IdCondicion
                'End If
            End If
        Catch ex As Exception
    ...
    Private Sub txtIgv_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtIgv.TextChanged, txtSubTotal.TextChanged, txtMontoNoAfecto.TextChanged
        CalcularMontos()
    End Sub

    Private Sub CalcularMontos()
        Dim Monto As Double
        Monto = IIf(cbAfectoIgv.Checked = True, (txtSubTotal.Value + txtMontoIgv.Value), txtSubTotal.Value)

        If cbAfectoIgv.Checked = True Then
            txtMontoIgv.Value = IIf(cbAfectoIgv.Checked = True, (txtSubTotal.Value * ((txtIgv.Value) / 100)), 0.0)
        Else
            txtMontoIgv.Value = 0
        End If
        txtMontoTotal.Value = txtSubTotal.Value + txtMontoIgv.Value + txtMontoNoAfecto.Value
        ...
        Try
            ...
            If e.Column.ActAsSelector Then
                ...
                If Check = False Then
                    ...
                    If (Session.CodPerfil = "01" Or Session.CodPerfil = "24") And (iCodJob <> "" Or iCodJob <> Nothing) And (estado = 3 Or estado = 5 Or estado = 7) And Not iProcesoJob Then
    ...
    'Private Sub cbComputo_CheckedChanged(sender As Object, e As System.EventArgs) Handles cbComputo.CheckedChanged
        ...
        Try
            If e.KeyChar = ChrW(Keys.Enter) Or e.KeyChar = ChrW(Keys.Tab) Then
                ...
                If Len(Trim(txtRuc.Text)) > 0 Then
                    If oProveedorService.BuscarRuc(Session.sCodEmp, Len(Trim(txtRuc.Text))) Then
                        Dim proveedor As New ProveedorService.Proveedor
                        proveedor = oProveedorService.ObtenerPorRuc(Session.sCodEmp, txtRuc.Text)
                        IdProveedor = proveedor.IdProveedor
                        txtProveedor.Text = proveedor.DesProv
                    Else
                        MsgBox("El R.U.C. ingresado no existe, verificar...", MsgBoxStyle.Information, "Información")
                        txtRuc.Text = ""
                        txtRuc.Focus()
                        IdProveedor = 0
                        txtProveedor.Text = ""
            ...
            End If
        Catch ex As Exception
            MsgBox("ERROR AL OBTENER EL PROVEEDOR : " + ex.Message, MsgBoxStyle.Exclamation)
        End Try
    End Sub

    Private Sub txtRuc_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtRuc.Click
        txtRuc.SelectAll()
    End Sub

    Private Sub txtXmlFE_KeyPress(sender As Object, e As System.Windows.Forms.KeyPressEventArgs) Handles txtXmlFE.KeyPress
        If e.KeyChar = ChrW(Keys.Enter) Or e.KeyChar = ChrW(Keys.Tab) Then
            e.Handled = True
            txtPdfFE.Focus()
        End If
    ...
    Private Sub txtPdfFE_KeyPress(sender As Object, e As System.Windows.Forms.KeyPressEventArgs) Handles txtPdfFE.KeyPress
        If e.KeyChar = ChrW(Keys.Enter) Or e.KeyChar = ChrW(Keys.Tab) Then
            e.Handled = True
