...
Public Class frmMemoTransferenciaInterna
    ...
    Private Sub enableOpciones()
        ...
        Else
            If state_button = True And toBlank(lblEstado.Text) = "GENERADO" Then
                ...
                miDescargarExcel.Enabled = True
                miImportarExcel.Enabled = True
            ...
            If MsgBox("¿Está seguro de IMPORTAR los datos?", MsgBoxStyle.YesNo, "Advertencia") = MsgBoxResult.Yes Then
                ...
                If dgvImportarExcel.RowCount > 0 Then
                    For Each fila As DataGridViewRow In dgvImportarExcel.Rows
                        ...
                        LocacionMercaderia = oLocacionMercaderiaService.MostrarPorCodigo(IdLocacion, Convert.ToString(fila.Cells("Codigo").Value))


                        registrodet.IdMemoDet = Nothing
                        memo.IdMemo = IdMemo
                        registrodet.Memo = memo
                        mercaderia.CodMer = IIf(Convert.ToString(fila.Cells("Codigo").Value) = "", Nothing, Convert.ToString(fila.Cells("Codigo").Value))
                        registrodet.Mercaderia = mercaderia
                        registrodet.CanMer = toNumber(fila.Cells("Cantidad").Value)
                        registrodet.PreMer = LocacionMercaderia.Mercaderia.DeaMer
                        registrodet.DscMer = 0.0
                        registrodet.TipMov = IIf(rbIngreso.Checked = True, "H", "D")
                        registrodet.CodUsu = Session.sCodUsu
                        registrodet.NomPc = Session.sNomPc
                        registrodet.DirIp = Session.sDirIp

                        estado_processdet = oMemoDetalleService.Insertar(registrodet)

                    Next

                    If estado_processdet > 0 Then
    ...
    Private Sub miDescargarExcel_Click(sender As System.Object, e As System.EventArgs) Handles miDescargarExcel.Click
        FormatoExcel()
    End Sub
    ...
    Private Sub FormatoExcel()
        ...
        Dim dtExcel As New DataTable("tabla2")
            ...
            dtExcel.Columns.Add(New DataColumn("Cantidad", Type.GetType("System.Int32")))
        ...
        'dtExcel.Rows.Add(New Object() {"1", "0", "0.00", "0.00"})
        dtExcel.Rows.Add(New Object() {"1", "0"})
