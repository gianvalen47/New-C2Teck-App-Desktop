Public Class frmMemosTransferenciasInternas
    ...
    Private Sub Initialize_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        oSeguridadService.InsertarSesionOpciones(Session.sIdSesion, 4)
        '/*************************************************************************************/
    ...
    Private Sub biAtender_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles biAtender.Click, miAtender.Click
        Try
            If MsgBox("¿Está seguro de ATENDER el Memo T.I. Nº " + dgvDatos.CurrentRow.Cells("NumDoc").Text.ToString + " ?", MsgBoxStyle.YesNo, "Información") = MsgBoxResult.Yes Then

                Dim lestado As String
                lestado = dgvDatos.CurrentRow.Cells("Estado").Text

                If lestado = "GENERADO" Then
                    Dim Atender As Boolean
                    Dim MemoRow As Integer
                    MemoRow = toNumber(dgvDatos.CurrentRow.Cells("IdMemo").Text)

                    Atender = oMemoService.Atender(toNumber(dgvDatos.CurrentRow.Cells("IdMemo").Text), Session.sCodUsu, Session.sNomPc, Session.sDirIp)

                    If Atender Then
                        MsgBox("Se Atendio el registro correctamente.", MsgBoxStyle.Information)
                        listaDatos()
                        RowPossesion(dgvDatos, MemoRow)
                    Else
                        MsgBox("No se Atendio el Memo de Transferencia interna.", MsgBoxStyle.Information)
                    End If
                Else
                    MsgBox("El Memo de transferencia interna debe estar en Generado", MsgBoxStyle.Information)
                End If
            End If
        Catch ex As Exception
            MsgBox("Error al ATENDER el Memo de Transferencia Interna : " + ex.Message, MsgBoxStyle.Exclamation)
        End Try
    End Sub
End Class