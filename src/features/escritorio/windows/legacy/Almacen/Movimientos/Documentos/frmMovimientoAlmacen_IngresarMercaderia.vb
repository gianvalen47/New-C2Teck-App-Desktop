...
Public Class frmMovimientoAlmacen_IngresarMercaderia
    ...
    Public IdLocacionI As Integer
    Public CodMerI As String
    Public DesMerI As String
    Public PreMerI As String
    ...
    Private Sub frmMovimientoAlmacen_IngresarMercaderia_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Insertar(ByVal registro As ProductoService.Producto)
        Try
            ...
            If estado_process = True Then
                ...
                Dim registroM As New LocacionMercaderiaService.LocacionMercaderia
                ...
                locacion.IdLocacion = toNull(IdLocacionI)
                empresa.CodEmp = Session.sCodEmp
                locacion.Empresa = empresa
                registroM.Locacion = locacion
                mercaderia.CodMer = toNull(txtCodMer.Text)
                registroM.Mercaderia = mercaderia
                registroM.UbiMer = toNull("")
                registroM.MinMer = toNull("")
                registroM.MaxMer = toNull("")
                registroM.ObsMer = toNull("")
                registroM.NomPc = Session.sNomPc
                registroM.DirIp = Session.sDirIp
                registroM.CodUsu = Session.sCodUsu

                Dim estado_processI As Boolean
                estado_processI = oLocacionMercaderiaService.Insertar(registroM)

                If estado_processI Then
                    CodMerI = txtCodMer.Text
                    DesMerI = txtDesMer1.Text
                    PreMerI = txtDeaMer.Text
                    Me.DialogResult = System.Windows.Forms.DialogResult.OK
                End If
                'End If
            Else
