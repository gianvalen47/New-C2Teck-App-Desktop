...
Public Class frmAlmacen_FacturaImportacion_AgregarDetalle
    ...
    Private Sub Initialize_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If state_button Then
            ObtenerRegistro()
            If estado_pedido = "GENERADO" Or estado_pedido = "GN" Or estado_pedido = "TRANSITO" Or estado_pedido = "TR" Or state_button = False Then
                '------------------- 25/05/2018 Se comenta, ya que solo se ingresan las F/I registrados en el módulo de Importación (Solo se módifica cantidades) ----------------
                'If oMaestroService.MostrarDato("SIGECOM.Almacen.Importaciones", "IdFactura", "IdImportacion", IdImportacion) <> Nothing Then
                '    If estado_pedido = "GENERADO" Or estado_pedido = "GN" Or estado_pedido = "TRANSITO" Or estado_pedido = "TR" Then
                '        btnGuardar.Enabled = True
                '    Else
                '        btnGuardar.Enabled = False
                '    End If
                '    desactivar()
                'Else
                'If estado_pedido = "TRANSITO" Or estado_pedido = "TR" Then
                btnGuardar.Enabled = True
                desactivar()
                'Else
                '    txtCodMer.ReadOnly = False
    ...
    Private Function ValidaCampos() As Boolean
        Try
            ...
            '    txtCanFac.Focus()
            '    Return False
            If toNumber(txtCanMer.Value) < 0 Then
    ...
    Private Sub btnGuardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGuardar.Click
        ...
        'registro.CanFac = txtCanFac.Value
        'registro.CanMer = txtCanMer.Value
        'registro.Item = txtItem.Value
        'registro.PreMer = txtPreMer.Value
        ...
        If MsgBox("¿Está seguro de ACTUALIZAR la cantidad recibida?", MsgBoxStyle.YesNo, "Advertencia") = MsgBoxResult.Yes Then
            If ValidaCampos() Then               
                Dim estado_process As Boolean
                estado_process = oImportacionDetService.ChequearCantidad(IdDetImportacion, toNumber(txtCanMer.Value))
                type_process = "insert"
                If estado_process > 0 Then
                    IdDetImportacion = IdDetImportacion
                    'IdDetPedidoImp = cmbPedidos.Value
                    Me.DialogResult = System.Windows.Forms.DialogResult.OK
                Else
                    MsgBox("Error en el proceso, comuníquese con el departamento de sistemas...!", MsgBoxStyle.Critical)
