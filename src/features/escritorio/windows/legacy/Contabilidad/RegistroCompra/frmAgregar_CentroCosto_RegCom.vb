...
Public Class frmAgregar_CentroCosto_RegCom
    ...
    Public MontoIgvSol As Double
    Public MontoNoAfectoSol As Double
    Public TotalSoles As Double

    Public MontoDolares As Double
    Public MontoIgvDol As Double
    Public MontoNoAfectoDol As Double
    Public TotalDolares As Double

    Private CodCentro As String
    Private DesCentro As String

    Private Sub frmAgregar_CentroCosto_RegCom_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub frmAgregar_CentroCosto_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        If e.KeyCode = Keys.Escape Then
            Me.DialogResult = System.Windows.Forms.DialogResult.Cancel
        End If
    End Sub

    Private Sub frmAgregar_CentroCosto_RegCom_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        Finalizar()
    End Sub
    ...
    Private Sub listaSeleccionados()
        Try
            ...
            '=================================== LISTA SELECCIONADOS ===================================          
            dtSeleccionados = oRegistroCompraDetService.MostrarCentroCosto(IdCompraDet).Tables(0)
            ...
            cMontoIgvSol.DataPropertyName = dtSeleccionados.Columns("MontoIgvSol").ColumnName
            cMontoNoAfectoSol.DataPropertyName = dtSeleccionados.Columns("MontoNoAfectoSol").ColumnName
            cMontoDol.DataPropertyName = dtSeleccionados.Columns("MontoDol").ColumnName
            cMontoIgvDol.DataPropertyName = dtSeleccionados.Columns("MontoIgvDol").ColumnName
            cMontoNoAfectoDol.DataPropertyName = dtSeleccionados.Columns("MontoNoAfectoDol").ColumnName
    ...
    Private Sub cbProrratear_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbProrratear.CheckedChanged
        ...
        Else
            If cbProrratear.Checked = True Then
                ...
                Dim MontoPIgvSol As Double
                Dim MontoPNoAfectoSol As Double

                Dim MontoPDol As Double
                Dim MontoPIgvDol As Double
                Dim MontoPNoAfectoDol As Double
                Dim Cont As Integer = 0
                ...
                MontoPIgvSol = MontoIgvSol / Cont
                MontoPNoAfectoSol = MontoNoAfectoSol / Cont

                MontoPDol = MontoDolares / Cont
                MontoPIgvDol = MontoIgvDol / Cont
                MontoPNoAfectoDol = MontoNoAfectoDol / Cont

                For i As Integer = 0 To dgvSeleccionados.RowCount - 1
                    dgvSeleccionados.Rows(i).Cells("cMontoSol").Value = MontoPSol
                    dgvSeleccionados.Rows(i).Cells("cMontoIgvSol").Value = MontoPIgvSol
                    dgvSeleccionados.Rows(i).Cells("cMontoNoAfectoSol").Value = MontoPNoAfectoSol

                    dgvSeleccionados.Rows(i).Cells("cMontoDol").Value = MontoPDol
                    dgvSeleccionados.Rows(i).Cells("cMontoIgvDol").Value = MontoPIgvDol
                    dgvSeleccionados.Rows(i).Cells("cMontoNoAfectoDol").Value = MontoPNoAfectoDol
                Next

            Else
                For i As Integer = 0 To dgvSeleccionados.RowCount - 1
                    dgvSeleccionados.Rows(i).Cells("cMontoSol").Value = 0.0
                    dgvSeleccionados.Rows(i).Cells("cMontoIgvSol").Value = 0.0
                    dgvSeleccionados.Rows(i).Cells("cMontoNoAfectoSol").Value = 0.0

                    dgvSeleccionados.Rows(i).Cells("cMontoDol").Value = 0.0
                    dgvSeleccionados.Rows(i).Cells("cMontoIgvDol").Value = 0.0
                    dgvSeleccionados.Rows(i).Cells("cMontoNoAfectoDol").Value = 0.0
                Next
            End If
            SumarMontos()
        End If
    ...
    Private Sub SumarMontos()
        Try
            If dgvSeleccionados.Rows.Count > 0 Then
                ...
                txtMontoIgvSol.Value = 0.0
                txtMontoNoAfectoSol.Value = 0.0

                txtMontoDolares.Value = 0.0
                txtMontoIgvDol.Value = 0.0
                txtMontoNoAfectoDol.Value = 0.0

                Dim MontoSolesAcum As Double = 0
                Dim MontoIgvSolAcum As Double = 0
                Dim MontoNoAfectoSolAcum As Double = 0

                Dim MontoDolaresAcum As Double = 0
                Dim MontoIgvDolAcum As Double = 0
                Dim MontoNoAfectoDolAcum As Double = 0
                ...
                For i As Integer = 0 To dgvSeleccionados.RowCount - 1
                    ...
                    MontoIgvSolAcum = MontoIgvSolAcum + IIf(IsDBNull(dgvSeleccionados.Rows(i).Cells("cMontoIgvSol").Value), 0.0, dgvSeleccionados.Rows(i).Cells("cMontoIgvSol").Value)
                    MontoNoAfectoSolAcum = MontoNoAfectoSolAcum + IIf(IsDBNull(dgvSeleccionados.Rows(i).Cells("cMontoNoAfectoSol").Value), 0.0, dgvSeleccionados.Rows(i).Cells("cMontoNoAfectoSol").Value)
                    ...
                    MontoIgvDolAcum = MontoIgvDolAcum + IIf(IsDBNull(dgvSeleccionados.Rows(i).Cells("cMontoIgvDol").Value), 0.0, dgvSeleccionados.Rows(i).Cells("cMontoIgvDol").Value)
                    MontoNoAfectoDolAcum = MontoNoAfectoDolAcum + IIf(IsDBNull(dgvSeleccionados.Rows(i).Cells("cMontoNoAfectoDol").Value), 0.0, dgvSeleccionados.Rows(i).Cells("cMontoNoAfectoDol").Value)
                Next

                txtMontoSoles.Value = Math.Round(MontoSolesAcum, 2)
                txtMontoIgvSol.Value = Math.Round(MontoIgvSolAcum, 2)
                txtMontoNoAfectoSol.Value = Math.Round(MontoNoAfectoSolAcum, 2)
                txtMontoTotalSol.Value = Math.Round((txtMontoSoles.Value + txtMontoIgvSol.Value + txtMontoNoAfectoSol.Value), 2)

                txtMontoDolares.Value = Math.Round(MontoDolaresAcum, 2)
                txtMontoIgvDol.Value = Math.Round(MontoIgvDolAcum, 2)
                txtMontoNoAfectoDol.Value = Math.Round(MontoNoAfectoDolAcum, 2)
                txtMontoTotalDol.Value = Math.Round((txtMontoDolares.Value + txtMontoIgvDol.Value + txtMontoNoAfectoDol.Value), 2)
            Else
                txtMontoSoles.Value = 0.0
                txtMontoIgvSol.Value = 0.0
                txtMontoNoAfectoSol.Value = 0.0
    ...
    Private Function ValidaBalanceMontos() As Boolean
        Try
            If txtMontoSoles.Value <> MontoSoles Or txtMontoDolares.Value <> MontoDolares Then
                ...
                dgvCentroCosto.Focus()
                Return False
            ElseIf txtMontoIgvSol.Value <> MontoIgvSol Or txtMontoIgvDol.Value <> MontoIgvDol Then
                MsgBox("Los Montos seleccionados deben ser iguales a los Montos del Detalle.", MsgBoxStyle.Information, "Información")
                dgvCentroCosto.Focus()
                Return False
            ElseIf txtMontoNoAfectoSol.Value <> MontoNoAfectoSol Or txtMontoNoAfectoDol.Value <> MontoNoAfectoDol Then
                MsgBox("Los Montos seleccionados deben ser iguales a los Montos del Detalle.", MsgBoxStyle.Information, "Información")
                dgvCentroCosto.Focus()
                Return False
                ...
                If ValidaCampos() Then
                    If ValidaBalanceMontos() Then
                        ...
                        For i As Integer = 0 To dgvSeleccionados.RowCount - 1
                            ...
                            rows(7) = dgvSeleccionados.Rows(i).Cells(4).Value
                            rows(8) = dgvSeleccionados.Rows(i).Cells(3).Value
                            rows(9) = dgvSeleccionados.Rows(i).Cells(5).Value
                            rows(10) = dgvSeleccionados.Rows(i).Cells(7).Value
                            rows(11) = dgvSeleccionados.Rows(i).Cells(6).Value
                            rows(12) = dgvSeleccionados.Rows(i).Cells(8).Value

                            dtTable.Rows.Add(rows)
                        Next

                        Dim state_process As Boolean
                        state_process = oRegistroCompraDetService.InsertarCentroCosto(IdCompraDet, IdCompra, dtTable, Session.sCodUsu, Session.sNomPc, Session.sDirIp)
