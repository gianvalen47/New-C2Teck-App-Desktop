...
Public Class frmBuscarPendientesMasivo
    ...
    Dim dtTableSeleccionados As DataTable
    ...
    Private Sub frmBuscarPendientes_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        dgvSeleccionados.ScrollBars = ScrollBars.Both
        dgvSeleccionados.AutoGenerateColumns = False
        dgvSeleccionados.Anchor = AnchorStyles.Bottom Or AnchorStyles.Top Or AnchorStyles.Right Or AnchorStyles.Left

        dtTableSeleccionados = dtDatos.Copy
        dtTableSeleccionados.Clear()
    ...
    '                    rows(13) = dgvDatos.Rows(i).Cells("cSaldoSol").Value
    '                    rows(14) = dgvDatos.Rows(i).Cells("cSaldoDol").Value
    ...
    Private Sub InsertarSeleccionados()
        Try
            If MsgBox("¿Está seguro de INSERTAR el(los) Doc(s) seleccionado(s)?", MsgBoxStyle.YesNo, "Advertencia") = MsgBoxResult.Yes Then

                If dtTableSeleccionados.Rows.Count = 0 Then
                    MsgBox("¡Debe seleccionar alguno de los Documentos...!", MsgBoxStyle.Information, "No hay datos")
                Else
                    Dim frm As New frmInsertarPendientes
                    frm.Provisional = False
                    If frm.ShowDialog = System.Windows.Forms.DialogResult.OK Then
                        dtDetalles = dtTableSeleccionados
                        IdDocumento = frm.IdDocumento
                        SerieDoc = frm.SerieDoc
                        NumDoc = frm.NumDoc
                        Nombre = dtTableSeleccionados.Rows(0).Item("DesProv").ToString
                        Me.DialogResult = System.Windows.Forms.DialogResult.OK
                        Me.Close()
                    End If
                End If
    ...
    Private Sub enableOpciones2()


        If dgvDatos.RowCount < 1 Then
            miAgregarSeleccionados.Enabled = False
            cmOpSeleccionados.Enabled = False
        Else
            cmOpSeleccionados.Enabled = True
            miAgregarSeleccionados.Enabled = True

        End If

        If dgvSeleccionados.RowCount < 1 Then
            miEliminarSeleccionado.Enabled = False
            miVaciarSeleccionados.Enabled = False
            miProcesarDocs.Enabled = False
            biInsertar.Enabled = False

        Else
            miEliminarSeleccionado.Enabled = True
            miVaciarSeleccionados.Enabled = True
            miProcesarDocs.Enabled = True
    ...
    Private Sub biInsertar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles biInsertar.Click
        InsertarSeleccionados()
    End Sub
    ...
    Private Sub miAgregarSeleccionados_Click(sender As Object, e As EventArgs) Handles miAgregarSeleccionados.Click
        dgvDatos.Update()
        'Dim dtTable As DataTable
        Dim rows As DataRow
        ...
        For i As Integer = 0 To dtDatos.Rows.Count - 1
            ...
            If toBoolean(cellSelecion.Value) = True Then
                rows = dtTableSeleccionados.NewRow
                rows(0) = dgvDatos.Rows(i).Cells("cIdCompra").Value
                rows(1) = dgvDatos.Rows(i).Cells("cIdCuenta").Value
                rows(2) = dgvDatos.Rows(i).Cells("cCodCuenta").Value
                rows(3) = dgvDatos.Rows(i).Cells("cIdDocumento").Value
                ...
                dtTableSeleccionados.Rows.Add(rows)

            End If
        Next

        dgvSeleccionados.DataSource = dtTableSeleccionados


        cIdCompra2.DataPropertyName = dtTableSeleccionados.Columns("IdCompra").ColumnName
        cIdCuenta2.DataPropertyName = dtTableSeleccionados.Columns("IdCuenta").ColumnName
        cCodCuenta2.DataPropertyName = dtTableSeleccionados.Columns("CodCuenta").ColumnName
        cIdDocumento2.DataPropertyName = dtTableSeleccionados.Columns("IdDocumento").ColumnName
        cSerDoc2.DataPropertyName = dtTableSeleccionados.Columns("SerDoc").ColumnName
        cTipMov2.DataPropertyName = dtTableSeleccionados.Columns("TipMov").ColumnName
        cNumDoc2.DataPropertyName = dtTableSeleccionados.Columns("NumDoc").ColumnName
        cIdProveedor2.DataPropertyName = dtTableSeleccionados.Columns("IdProveedor").ColumnName
        cDocumento2.DataPropertyName = dtTableSeleccionados.Columns("Documento").ColumnName
        cDesProv2.DataPropertyName = dtTableSeleccionados.Columns("DesProv").ColumnName
        cFecDoc2.DataPropertyName = dtTableSeleccionados.Columns("FecDoc").ColumnName
        cCodMon2.DataPropertyName = dtTableSeleccionados.Columns("CodMon").ColumnName
        cSaldoSol2.DefaultCellStyle.Format = "N2"
        cSaldoSol2.DataPropertyName = dtTableSeleccionados.Columns("SaldoSol").ColumnName
        cSaldoDol2.DefaultCellStyle.Format = "N2"
        cSaldoDol2.DataPropertyName = dtTableSeleccionados.Columns("SaldoDol").ColumnName
        cFecVen2.DataPropertyName = dtTableSeleccionados.Columns("FecVen").ColumnName
        'cSeleccion.DataPropertyName = dtTableSeleccionados.Columns("Seleccion").ColumnName
        cIdCliente2.DataPropertyName = dtTableSeleccionados.Columns("IdCliente").ColumnName
        cIdHonorario2.DataPropertyName = dtTableSeleccionados.Columns("IdHonorario").ColumnName
    ...
    Private Sub EliminarFila(ByVal dgvDatos As DataGridView)
        dgvSeleccionados.Rows.Remove(dgvDatos.CurrentRow)
        enableOpciones2()
    End Sub


    Private Sub miEliminarSeleccionado_Click(sender As Object, e As EventArgs) Handles miEliminarSeleccionado.Click
        EliminarFila(dgvSeleccionados)
    End Sub

    Private Sub miVaciarSeleccionados_Click(sender As Object, e As EventArgs) Handles miVaciarSeleccionados.Click
        dtTableSeleccionados.Clear()
        dgvSeleccionados.DataSource = dtTableSeleccionados
        enableOpciones2()
    End Sub

    Private Sub miProcesarDocs_Click(sender As Object, e As EventArgs) Handles miProcesarDocs.Click
        InsertarSeleccionados()
    End Sub
End Class
