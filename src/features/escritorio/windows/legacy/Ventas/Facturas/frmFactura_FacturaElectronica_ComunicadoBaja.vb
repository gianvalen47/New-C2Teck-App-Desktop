...
Public Class frmFactura_FacturaElectronica_ComunicadoBaja
    ...
    Private Sub frmFactura_FacturaElectronica_ComunicadoBaja_FormClosed(sender As Object, e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        Try
    ...
    Private Sub frmFactura_FacturaElectronica_ComunicadoBaja_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub frmFactura_FacturaElectronica_ComunicadoBaja_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load

        Try
            Dim dtDarBaja As New DataTable

            dtDarBaja = oFacturaService.ComunicacionBajaDigital(IdFactura).Tables(0)
    ...
    Private Sub CrearXmlSerializado()
        Try
            ...
            'Dim xmlFilename As String = "D:\Documentos_Electronicos\Comunicacion_de_Baja\" & "FF11-119026" & "\201000-20441-RA-" & FecDoc.ToString("yyyyMMdd") & "-1" & ".xml"
            ...
            'FacturacionElectronica.GenerarVoidedDocuments(xmlFilename, "20100020441", "DETROIT DIESEL - MTU PERU S.A.C.", CurrencyId, "RA-" & FecDocForm & "-" & NumCorrelativo, Today.Date, FecDoc, line1)
            ...
            'direccion = "D:\Documentos_Electronicos\Facturas_Electronicas\" & GridEX1.CurrentRow.Cells("NumDoc").Text & "\201000-20441-01-" & GridEX1.CurrentRow.Cells("NumDoc").Text & ".xml"

            Dim ruta As String = System.AppDomain.CurrentDomain.BaseDirectory & "Certificado\" & Session.sNombreCertificado ' CertificadoFacturacion.pfx"
            'Dim ruta As String = System.AppDomain.CurrentDomain.BaseDirectory & "Certificado\" & "CertificadoFacturacion.pfx"


            'firmado = ObjLib.SignXmlFile("201000-20441", "D:\Documentos_Electronicos\xadesnettest.p12", "xadesnet", direccion)
            'activar -------------------------------------------------------------------------------------------------------------------------------------------------------------

            'firmado = ObjLib.SignXmlFile(Session.sRucEmp, 5, ruta, Session.sClaveCertificado, direccion)

            firmado = FirmarDocumento21.SignXmlFile(Session.sRucEmp, 5, ruta, Session.sClaveCertificado, direccion)
            ...
            If firmado = True Then
                ...
                Dim file As String = Session.sRucEmp & "-RA-" & FecDocForm & "-" & NumCorrelativo
                'Dim file As String = "20100020441-RA-" & FecDocForm & "-" & NumCorrelativo
                ...
                '    numeroTicket = EnviarSunat21.EnviarComunicacionBajaOSE(Session.sRucEmp, Session.sUsuarioOSE, Session.sClaveOSE, rutaenvio, file)
                'Else
                '    'file = Session.sRucEmp & "-RA-" & FecDocForm & "-" & NumCorrelativo
                '    numeroTicket = EnviarSunat.EnviarComunicacionBajaProduccion(Session.sRucEmp, Session.sUsuarioSunat, Session.sClaveSunat, rutaenvio, file)
                'End If

                If Session.sAplicaOSE Then
                    numeroTicket = EnviarSunat21.EnviarComunicacionBajaOSE(Session.sRucEmp, Session.sUsuarioOSE, Session.sClaveOSE, rutaenvio, file)
                Else
                    numeroTicket = EnviarSunat21.EnviarComunicacionBajaProduccion(Session.sRucEmp, Session.sUsuarioSunat, Session.sClaveSunat, rutaenvio, file)
                End If
                ...
                'Dim nombrexml As String = "201000-20441-01-" & GridEX1.CurrentRow.Cells("NumDoc").Text '& ".xml"
                ...
                If insertar = True Then
                    ...
                    '        xmlDocR.Load("D:\Documentos_Electronicos\Comunicacion_de_Baja\" & GridEX1.CurrentRow.Cells("Documento").Text & "\R-201000-20441-RA-" & FecDocForm & "-" & NumCorrelativo & ".xml")
    ...
    Private Sub btnAceptar_Click(sender As System.Object, e As System.EventArgs) Handles btnAceptar.Click
        Try
            ...
            Else
                'Dim comunicadobajaexiste As Boolean
                If MsgBox("¿Está seguro de generar la comunicación de baja del comprobante N° " & Documento & "?", MsgBoxStyle.YesNo, "Advertencia") = MsgBoxResult.Yes Then
                    'FecDocForm = FecDoc.ToString("yyyyMMdd")
                    'NumCorrelativo = oComunicacionBajaDigitalService.ObtenerNumero(Session.sCodEmp)

                    'comunicadobajaexiste = oComunicacionBajaDigitalService.Buscar(Session.sCodEmp, "RA-" & FecDocForm & "-" & NumCorrelativo)

                    'If comunicadobajaexiste = True Then
                    '    MsgBox("El comunicado de baja ya existe, tenga cuidado", MsgBoxStyle.Information)
                    'Else
                    CrearCarpeta()
                    CrearXmlSerializado()
