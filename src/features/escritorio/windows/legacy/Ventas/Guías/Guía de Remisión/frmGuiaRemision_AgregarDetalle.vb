...
Public Class frmGuiaRemision_AgregarDetalle
    ...
    Public CodMot As String    '----------Agregado para validar que cuando sea motivo Transferencia Gratuita ('2') el precio debe de ser el Costo (SOLO PARA ESTE MOTIVO) y no el precio de venta 
    ...
    Private Sub txtCodMer_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtCodMer.KeyDown
        If e.KeyCode = Keys.F12 Then
            If txtCodMer.ButtonEnabled = True Then
                txtCodMer_ButtonClick(sender, e)
            End If
        End If
    End Sub
    '====================================================================================================================
    ...
    Private Sub SendFocus_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles _
                            txtCodMer.KeyPress, txtStock.KeyPress, txtItem.KeyPress, txtDesMer.KeyPress, _
                            txtCanMer.KeyPress, txtPreMer.KeyPress, txtDscMer.KeyPress, txtTotal.KeyPress
    ...
    Private Sub frmGuiaRemision_AgregarDetalle_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub Initialize_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Else                    'Nuevo
            ...
            txtCodMer.Enabled = True
            txtItem.Value = oGuiaRemisionDetService.SugerirItem(IdGuia)
            cbNoCore.Checked = False
            txtCodMer.Select()
        End If

        If estado = "GENERADO" Or estado = "GN" Or state_button = False Or estado = "APROBADO" Then
            ...
            '============== Se agregó el 19/05/2014 Jacquelin 3804 ==============
            If oGuiaRemisionService.BuscarOrden(IdGuia) Then
              activar
                btnGuardar.Enabled = True            
            End If
            '==========================================================
    ...
    Private Function ValidaCampos() As Boolean
        Try
            ...
            ElseIf toNumber(txtCanMer.Value) <= 0 Then
                ...
                txtCanMer.Focus()
                Return False
                'ElseIf txtPreMer.Text.Trim.Length > 0 And CDbl(txtPreMer.Text.Trim) <= 0 And TipMov <> "O" And CDbl(txtPrecioSug.Text.Trim) <= 0 And Mid(Trim(txtCodMer.Text), 1, 3) <> "AAA" Then
            ...
            ElseIf txtDesMer.Text = "" Then
                MsgBox("Debe Ingresar la descripción del artículo ", MsgBoxStyle.Information, "Información")
                txtDesMer.BackColor = Color.Red
                txtDesMer.Focus()
                Return False
            ElseIf state_button = False And toNumber(txtStock.Text) <= 0 And TipMov = "D" And Mid(Trim(txtCodMer.Text), 1, 3) <> "AAA" Then
                MsgBox("No hay STOCK para esta mercadería. ", MsgBoxStyle.Information, "Información")
                txtCodMer.Text = ""
                txtCodMer.Focus()
                Return False
            ElseIf state_button = False And toNumber(txtCanMer.Value) > toNumber(txtStock.Text) And TipMov = "D" And Mid(Trim(txtCodMer.Text), 1, 3) <> "AAA" Then
                MsgBox("Cantidad no pueder ser mayor que Stock. ", MsgBoxStyle.Information, "Información")
                txtCanMer.Focus()
                Return False
            ElseIf state_button = False And (oGuiaRemisionDetService.Buscar(IdGuia, toBlank(txtCodMer.Text)) And cbRegalo.Checked = False) And Mid(Trim(txtCodMer.Text), 1, 3) <> "AAA" Then
            ...
            ElseIf cbSugerir.Checked = True And txtDsctoSug.Value < 0 Then
                ...
                txtPrecioSug.Focus()
                Return False
            ElseIf state_button = True And oGuiaRemisionService.Estado(IdGuia) <> "GENERADO" And oGuiaRemisionService.Estado(IdGuia) <> "APROBADO" Then
    ...
    Private Sub desactivar()
        ...
        cbRegalo.Enabled = False
    End Sub
    Private Sub Insertar(ByVal registro As GuiaRemisionDetService.GuiaRemisionDet)
        Try
            Dim estado_process As Integer
            estado_process = oGuiaRemisionDetService.Insertar(registro)
            type_process = "insert"
            If estado_process > 0 Then
                IdGuiaDet = estado_process
                If cbSugerir.Checked = True Then
                    oGuiaRemisionDetService.InsertarSugerido(IdGuiaDet, IdGuia, txtPrecioSug.Value, txtDsctoSug.Value)
        ...
        End Try
    End Sub
    Private Sub Modificar(ByVal registro As GuiaRemisionDetService.GuiaRemisionDet)
        Try
            Dim estado_process As Boolean
            estado_process = oGuiaRemisionDetService.Actualizar(registro)
            type_process = "update"
            If estado_process = True Then
                If cbSugerir.Checked = True And IdSugerido > 0 Then
                    oGuiaRemisionDetService.ActualizarSugerido(IdSugerido, IdGuiaDet, IdGuia, txtPrecioSug.Value, txtDsctoSug.Value)
        ...
        End Try
    End Sub
    Private Sub ModificarDescripcion(ByVal registro As GuiaRemisionDetService.GuiaRemisionDet)
        Try
            Dim estado_process As Boolean
            estado_process = oGuiaRemisionDetService.ActualizarDescripcion(registro.IdGuiaDet, registro.GuiaRemision.IdGuia, txtDesMer.Text, Session.sCodUsu, Session.sNomPc, Session.sDirIp)
            type_process = "update"
            If estado_process = True Then
                'If cbSugerir.Checked = True And IdSugerido > 0 Then
                '    oGuiaRemisionDetService.ActualizarSugerido(IdSugerido, IdGuiaDet, IdGuia, txtPrecioSug.Value, txtDsctoSug.Value)
    ...
    Private Sub ObtenerRegistro()
        Try
            ...
            If IdSugerido > 0 Then
                ...
                cbSugerir.Enabled = False
                Dim Sugerido As GuiaRemisionDetService.SugeridoGuiaDet
                Sugerido = oGuiaRemisionDetService.MostrarSugeridoPorId(IdSugerido, IdGuiaDet)
    ...
    Private Sub btnGuardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGuardar.Click
        If MsgBox("¿Está seguro de GUARDAR los datos?", MsgBoxStyle.YesNo, "Advertencia") = MsgBoxResult.Yes _
            ...
            Dim registro As New GuiaRemisionDetService.GuiaRemisionDet
            Dim guia As New GuiaRemisionDetService.GuiaRemision
            Dim mercaderia As New GuiaRemisionDetService.Mercaderia

            registro.IdGuiaDet = IIf(toNumber(IdGuiaDet) = 0, Nothing, IdGuiaDet)
            guia.IdGuia = IIf(toNumber(IdGuia) = 0, Nothing, IdGuia)
        ...
        End If
    End Sub
    Private Sub txtCodMer_ButtonClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtCodMer.ButtonClick
            ...
            Try
                If Len(Trim(txtCodMer.Text)) > 0 Then
                    ...
                    If oLocacionMercaderiaService.Buscar(IdLocacion, txtCodMer.Text) Then
                        ...
                        If oPermisousuario.Precio = False Then
                            If Mid(Trim(txtCodMer.Text), 1, 3) = "AAA" Then '  Trim(txtCodMer.Text).Substring(0, 3) = "AAA" Then
                                txtPreMer.ReadOnly = False
                                txtDscMer.ReadOnly = False
                                txtPreMer.BackColor = System.Drawing.SystemColors.Window
                                txtDscMer.BackColor = System.Drawing.SystemColors.Window
    ...
    Private Sub cbRegalo_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cbRegalo.CheckedChanged
        ...
        Catch ex As Exception
            MsgBox("Error al Poner Mercadería como Regalo" + ex.Message)
        End Try
    End Sub

    Private Sub cbNoCore_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cbNoCore.CheckedChanged

    End Sub

    Private Sub txtCodMer_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtCodMer.Click
