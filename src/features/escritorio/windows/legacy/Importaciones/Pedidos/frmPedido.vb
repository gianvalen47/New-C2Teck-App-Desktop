  ...
  '====================================================================================================================
    ...
    Private Sub Initialize_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Else                    'Nuevo
            ...
            Me.Size = New System.Drawing.Size(790, 265)
    ...
    Private Function ValidaCampos() As Boolean
        Try
            ...
            ElseIf toNumber(cmbIdPer.Value) = 0 Then
                ...
                txtFecha.Focus()
                Return False
            ElseIf toNull(cmbCodMon.text) = "" Then
    ...
    Private Sub biGuardar1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles biGuardar1.Click
        ...
        And ValidaCampos() Then
            ...
            Dim NumeroADR As New PedidoService.NumerosADR
            Dim moneda As New PedidoService.Moneda
    ...
    Private Sub InsertarMasivo()
        Try
            ...
            dtDetalles = GetDataExcel(DirFile, fileExt)
            ...
            dtCopia.Columns.Add(New DataColumn("CanPed", Type.GetType("System.Int32")))       'datetime
            dtCopia.Columns.Add(New DataColumn("PreMer", Type.GetType("System.Double")))        'datetime
            dtCopia.Columns.Add(New DataColumn("Destino", Type.GetType("System.String")))
            dtCopia.Columns.Add(New DataColumn("Observacion", Type.GetType("System.String")))

            dtCopia.Rows.Add(New Object() {"1", "1", "1", "0.00", "1", "1"})

            dtDatosN = dtCopia.Copy
            dtDatosN.Clear()

            For Each row2 As DataRow In dtdatos.Rows

                If Not IsDBNull(row2("CodMer")) Then    'CStr(row2("CodMer")) <> "" Or 

                    row = dtDatosN.NewRow

                    Dim desm As String = ""
                    Dim destinom As String = ""
                    Dim obs As String = ""
                    If IsDBNull(row2("DesMer")) Then
                        desm = ""
                    Else desm = CStr(row2("DesMer"))
                    End If
                    If IsDBNull(row2("Destino")) Then
                        MsgBox("El campo destino es obligatorio", MsgBoxStyle.Exclamation)
                        Exit Sub
                    End If

                    If IsDBNull(row2("Observacion")) Then
                        obs = ""
                    Else obs = CStr(row2("Observacion"))
                    End If



                    row(0) = CStr(row2("CodMer"))
                    row(1) = desm 'IIf(IsDBNull(row2("DesMer")), "", CStr(row2("DesMer")))
                    row(2) = CInt(row2("CanPed"))
                    row(3) = CDbl(row2("PreMer"))
                    row(4) = CStr(row2("Destino"))
                    row(5) = obs 'CStr(row2("Observacion"))
            ...
            state_process = oPedidoDetService.InsertarMasivo(toNumber(txtIdPedido.Text), dtDatosN, Session.sCodUsu, Session.sNomPc, Session.sDirIp)

            If state_process > 0 Then
                listaDatos()
            Else
                MsgBox("Error al cargar Excel", MsgBoxStyle.Information)
            End If
