...
Public Class frmFaltaPersonal_Imprimir
    ...
    Private CodMotivo As String

    Public iCodArea As String
    Public iCodCentro As String
    Public iCodMotivo As String

    Private Sub frmFaltaPersonal_Imprimir_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        Finalizar()
    End Sub

    Private Sub frmFaltaPersonal_Imprimir_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub frmFaltaPersonal_Imprimir_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub listaDatos()
        Try
            dtDatos = oFaltasService.ImprimirGrupo(Session.sCodEmp, cmbCodArea.Value, cmbCentroCosto.Value, cmbMotivo.Value, txtFecha.Value, toNumber(cmbPerAutoriza.Value)).Tables(0)
            ...
            cIdFalta.DataPropertyName = dtDatos.Columns("IdFalta").ColumnName
            ...
            cCodMotivo.DataPropertyName = dtDatos.Columns("CodMotivo").ColumnName
            cDesMotivo.DataPropertyName = dtDatos.Columns("DesMotivo").ColumnName
            cIdPer.DataPropertyName = dtDatos.Columns("IdPer").ColumnName
            cApeNom.DataPropertyName = dtDatos.Columns("ApeNom").ColumnName
            cFecInicio.DataPropertyName = dtDatos.Columns("FecInicio").ColumnName
            cFecFinal.DataPropertyName = dtDatos.Columns("FecFinal").ColumnName
            cHorInicio.DataPropertyName = dtDatos.Columns("HorInicio").ColumnName
            cHorFinal.DataPropertyName = dtDatos.Columns("HorFinal").ColumnName
            cObservacion.DataPropertyName = dtDatos.Columns("Observacion").ColumnName
            cPagado.DataPropertyName = dtDatos.Columns("Pagado").ColumnName
    ...
    Private Sub biImprimir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles biImprimir.Click
        ...
        Try
            ...
            Dim reporte As New rptFaltaPersonalCentroCosto
            Dim reportesinFirma As New rptFaltaPersonalCentroCostoSinFirma
            Dim dtReporte As New DataTable

            dtReporte = dtTable

            CodMotivo = cmbMotivo.Value

            If CodMotivo = "09" Then

                If dtReporte.Rows.Count = 0 Then
                    MsgBox("!Debe seleccionar al menos un registro...!", MsgBoxStyle.Information, "No hay datos")
                Else
                    reportesinFirma.SetDataSource(dtReporte)
                    forma.crvReportes.ReportSource = reportesinFirma
                    ...
                    End If
                    reportesinFirma.SetParameterValue("pFecha", txtFecha.Value)
                    forma.Text = "Reporte de Faltas de Personal por Centro de Costo"
                    forma.ShowDialog()
                End If
    ...
    Private Sub btnBuscar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBuscar.Click, txtFecha.ValueChanged, cmbMotivo.ValueChanged, cmbPerAutoriza.ValueChanged
        listaDatos()
    End Sub
End Class