...
Public Class frmRepTardanzas
    ...
    Private Sub frmRepTradanzas_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        Finalizar()
    End Sub
    ...
    Private Sub frmRepTradanzas_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub frmRepTradanzas_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load

        '/************************** Insertar Opciones de Session ************************/
        oSeguridadService.InsertarSesionOpciones(Session.sIdSesion, 218)
        '/*************************************************************************************/
    ...
    Private Sub btnAceptar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAceptar.Click
        Try
            If ValidaCampos() Then
                ...
                oSeguridadService.RegistrarVisitaOpciones(218, "SIGECOM", Session.sCodUsu, Session.sNomPc, Session.sDirIp)

                If rbEntrada.Checked = True Then

                    Dim forma As New frmReportes
                    Dim reporte As New rptRepTardanzasEnt
                    Dim dtReporte As New DataTable

                    oMarcacionService.InnerChannel.OperationTimeout = TimeSpan.FromMinutes(20)
                    dtReporte = oMarcacionService.ReporteTardanza(Session.sCodEmp, toBlank(cmbArea.Value), toBlank(cmbCentroCosto.Value), toBlank(cmbClase.Value), txtFecInicio.Value,
                                                                                            txtFecFinal.Value, IdPersona, IIf(cbMarca.Checked = True, True, False), 1).Tables(0)
                    ...
                    Else
                        ...
                        End If
                        forma.Text = "Reporte de Tardanza de Entrada"
                ...
                ElseIf rbRefrigerio.Checked = True Then
                    ...
                    Dim forma As New frmReportes
                    Dim reporte As New rptRepTardanzasRef
                    Dim dtReporte As New DataTable
                    oMarcacionService.InnerChannel.OperationTimeout = TimeSpan.FromMinutes(20)
                    dtReporte = oMarcacionService.ReporteTardanza(Session.sCodEmp, toBlank(cmbArea.Value), toBlank(cmbCentroCosto.Value), toBlank(cmbClase.Value), txtFecInicio.Value,
                                                                                            txtFecFinal.Value, IdPersona, IIf(cbMarca.Checked = True, True, False), 2).Tables(0)
                    ...
                    Else
                        ...
                        Else
                            forma.crvReportes.ShowExportButton = False
                        End If
                        forma.Text = "Reporte de Tardanza de Refrigerio"
                        reporte.SetParameterValue("pFecInicio", txtFecInicio.Value)
                        reporte.SetParameterValue("pFecFinal", txtFecFinal.Value)
                        reporte.SetParameterValue("pArea", IIf(cmbArea.SelectedIndex = 0, "", UCase(cmbArea.Text)))
                        reporte.SetParameterValue("pCentro", UCase(cmbCentroCosto.Text))
                ...
                ElseIf rbConsolidado.Checked = True Then

                    Dim forma As New frmReportes
                    Dim reporte As New rptRepTardanzasCons
                    Dim dtReporte As New DataTable
                    oMarcacionService.InnerChannel.OperationTimeout = TimeSpan.FromMinutes(20)
                    dtReporte = oMarcacionService.ReporteTardanza(Session.sCodEmp, toBlank(cmbArea.Value), toBlank(cmbCentroCosto.Value), toBlank(cmbClase.Value), txtFecInicio.Value,
                                                                                            txtFecFinal.Value, IdPersona, IIf(cbMarca.Checked = True, True, False), 3).Tables(0)
                ...
                ElseIf rbRecord.Checked = True Then

                    Dim forma As New frmReportes
                    Dim reporte As New rptRepTardanzasRec
                    Dim dtReporte As New DataTable
                    oMarcacionService.InnerChannel.OperationTimeout = TimeSpan.FromMinutes(20)
                    dtReporte = oMarcacionService.ReporteTardanza(Session.sCodEmp, toBlank(cmbArea.Value), toBlank(cmbCentroCosto.Value), toBlank(cmbClase.Value), txtFecInicio.Value,
                                                                                            txtFecFinal.Value, IdPersona, IIf(cbMarca.Checked = True, True, False), 4).Tables(0)
                ...
                ElseIf rbPuntualidad.Checked = True Then

                    Dim forma As New frmReportes
                    Dim reporte As New rptRepTardanzasPunt
                    Dim dtReporte As New DataTable
                    oMarcacionService.InnerChannel.OperationTimeout = TimeSpan.FromMinutes(20)

                    dtReporte = oMarcacionService.ReporteTardanza(Session.sCodEmp, toBlank(cmbArea.Value), toBlank(cmbCentroCosto.Value), toBlank(cmbClase.Value), txtFecInicio.Value,
                                                                                            txtFecFinal.Value, IdPersona, IIf(cbMarca.Checked = True, True, False), 5).Tables(0)
                    ...
                    Else
                        ...
                        Else
                            forma.crvReportes.ShowExportButton = False
                        End If
                        forma.Text = "Reporte de Puntualidad"
                        reporte.SetParameterValue("pFecInicio", txtFecInicio.Value)
                        reporte.SetParameterValue("pFecFinal", txtFecFinal.Value)
                        reporte.SetParameterValue("pArea", IIf(cmbArea.SelectedIndex = 0, "", UCase(cmbArea.Text)))
                        reporte.SetParameterValue("pCentro", UCase(cmbCentroCosto.Text))
