...
const rows = [
  { numero: "TI-001", fecha: "2026-09-08", cliente: "LUCY VENTAS S.A.C.", tipo: "Memo de Tran", moneda: "PEN", total: "1,248.00", estado: "GENERADO", aprobador: "JORGE L" },
  { numero: "TI-002", fecha: "2026-09-08", cliente: "MERCADOS DEL SUR", tipo: "Generado", moneda: "PEN", total: "2,430.50", estado: "APROBADO", aprobador: "PATRICIA R" },
  { numero: "TI-003", fecha: "2026-09-07", cliente: "INVERSIONES PUNTO DOC", tipo: "Memo de Tran", moneda: "USD", total: "518.25", estado: "ANULADO", aprobador: "—" },
];

export function MemosTransferenciasInternasList() {
  return (
    <div className="h-full flex flex-col bg-[#EEF3F8] text-slate-800">
      <div className="flex items-center justify-between px-3 py-1.5 bg-gradient-to-b from-[#3E5B7A] to-[#2A3F55] border-b border-[#1F2E3F] text-[12px] text-white">
        <span className="font-medium">Memos de Transferencias Internas</span>
      ...
      <div className="px-2 py-2 border-b border-slate-300 bg-[#F7FAFC]">
        <div className="grid grid-cols-2 xl:grid-cols-8 gap-2">
          ...
          <Field label="Tipo" className="min-w-[150px]"><select className={inp}><option>Memo de Tran</option><option>Generado</option></select></Field>
      ...
      <div className="px-2 py-2">
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-2 text-[11px]">
          <div className="rounded-sm border border-slate-300 bg-white px-2 py-1"><span className="text-slate-600">Generados:</span> <b className="font-mono text-slate-900">3</b></div>
          <div className="rounded-sm border border-slate-300 bg-white px-2 py-1"><span className="text-slate-600">Aprobados:</span> <b className="font-mono text-emerald-700">1</b></div>
          ...
          <div className="rounded-sm border border-slate-300 bg-white px-2 py-1"><span className="text-slate-600">Total S/:</span> <b className="font-mono text-slate-900">4,197.00</b></div>
        </div>
      </div>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.numero} className="hover:bg-slate-50">
                  ...
                  <td className="border-b border-slate-200 px-2 py-1.5">{row.aprobador}</td>
