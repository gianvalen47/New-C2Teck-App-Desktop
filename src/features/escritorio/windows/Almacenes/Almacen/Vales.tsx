...
function EstadoBadge({ estado }: { estado: string }) {
  const map: Record<string, string> = {
    ...
    DESPACHADO: "text-emerald-700 bg-emerald-50 border-emerald-200",
...
const rows = [
  { numero: "V-0001", ot: "OT-2458", fecha: "2026-09-08", cliente: "LUCY VENTAS S.A.C.", moneda: "PEN", total: "1,248.00", estado: "GENERADO", prioridad: "ALTA" },
  { numero: "V-0002", ot: "OT-2141", fecha: "2026-09-08", cliente: "MERCADOS DEL SUR", moneda: "PEN", total: "2,430.50", estado: "DESPACHADO", prioridad: "MEDIA" },
  { numero: "V-0003", ot: "OT-1889", fecha: "2026-09-07", cliente: "INVERSIONES PUNTO DOC", moneda: "USD", total: "518.25", estado: "CERRADO", prioridad: "BAJA" },
  { numero: "V-0004", ot: "OT-1733", fecha: "2026-09-06", cliente: "ALMACEN DEL NORTE", moneda: "PEN", total: "3,600.00", estado: "ANULADO", prioridad: "MEDIA" },
];
    ...
    <div className="h-full flex flex-col bg-[#EEF3F8] text-slate-800">
      <div className="flex items-center justify-between px-3 py-1.5 bg-gradient-to-b from-[#3E5B7A] to-[#2A3F55] border-b border-[#1F2E3F] text-[12px] text-white">
        ...
        <div className="flex items-center gap-1">
          <button className={iconBtn} title="Nuevo vale"><Plus className="h-4 w-4 text-white" /></button>
          {tbSep}
      ...
      <div className="px-2 py-2 border-b border-slate-300 bg-[#F7FAFC]">
        <div className="grid grid-cols-2 xl:grid-cols-8 gap-2">
          ...
          <Field label="Tipo" className="min-w-[140px]"><select className={inp}><option>Vale de Almacén</option><option>Despacho</option><option>Requisición</option></select></Field>
      ...
      <div className="px-2 py-2">
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-2 text-[11px]">
          <div className="rounded-sm border border-slate-300 bg-white px-2 py-1"><span className="text-slate-600">Pendientes:</span> <b className="font-mono text-amber-700">1</b></div>
          <div className="rounded-sm border border-slate-300 bg-white px-2 py-1"><span className="text-slate-600">Despachados:</span> <b className="font-mono text-emerald-700">1</b></div>
          <div className="rounded-sm border border-slate-300 bg-white px-2 py-1"><span className="text-slate-600">Cerrados:</span> <b className="font-mono text-slate-900">1</b></div>
          <div className="rounded-sm border border-slate-300 bg-white px-2 py-1"><span className="text-slate-600">Monto S/:</span> <b className="font-mono text-slate-900">7,997.75</b></div>
        </div>
      </div>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.numero} className="hover:bg-slate-50">
                  ...
                  <td className="border-b border-slate-200 px-2 py-1.5 text-slate-600">{row.prioridad}</td>
                </tr>
              ))}
            </tbody>
          </table>
