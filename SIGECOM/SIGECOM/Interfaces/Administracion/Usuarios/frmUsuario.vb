...
Public Class frmUsuario
    ...
    Private Sub frmUsuario_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If state_button Then    'Modificar 
            ...
            Me.Size = New System.Drawing.Size(851, 555)
            dgvSistemas.Select()
        Else                          'Nuevo
            Me.Size = New System.Drawing.Size(750, 224)
    ...
    Private Sub desactivar()
        ...
        txtTelefonos.BackColor = System.Drawing.SystemColors.Control
        lblFecModificacion.Visible = True
        txtFecMod.Visible = True
        cbVigente.Enabled = False
        cbSeteo.Enabled = False
        cbAlertas.Enabled = False
        cbMarcaSoloJob.Enabled = False
        edicion = False
        enableOpciones()
        txtCodUsu.Focus()
    End Sub
    ...
    Private Sub ObtenerRegistro()
        Try
            ...
            cbMarcaSoloJob.Checked = registro.SoloMarcaJob
    ...
    Private Sub biGuardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles biGuardar.Click
        Try
            ...
            And ValidaCampos() Then
                ...
                registro.SoloMarcaJob = cbMarcaSoloJob.Checked
    ...
    Private Sub EnableOptionsOptions()
        Try
            ...
            If dgvEquipos.RowCount > 0 Then
                miEliminarEquipo.Enabled = True
            Else
                miEliminarEquipo.Enabled = False
            End If
    ...
    Private Sub ObtenerPermisos()
        Try
            If oSeguridadService.BuscarPermiso(CodUsu) Then
                ...
                ckExportarDatos.Checked = registro.ExpDatos
                ckAprobacionUnica.Checked = registro.AprobacionUnica
                ckLibreLicencia.Checked = registro.LibreLicencia
    ...
    Private Sub miEliminarLoc_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles miEliminarLoc.Click
        Try
            ...
            If MsgBox("¿Está seguro de ELIMINAR el registro seleccionado?", MsgBoxStyle.YesNo, "Información") = MsgBoxResult.Yes Then
                Dim estado_process As Boolean
                estado_process = oSeguridadService.BorrarLocacionUsuario(CodUsu, toNumber(dgvLocaciones.CurrentRow.Cells("IdLocacion").Value), Session.sCodUsu, Session.sNomPc, Session.sDirIp)
                If estado_process = True Then
    ...
    Private Sub miEliminarCentro_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles miEliminarCentro.Click
        Try
            ...
            If MsgBox("¿Está seguro de ELIMINAR el registro seleccionado?", MsgBoxStyle.YesNo, "Información") = MsgBoxResult.Yes Then
                Dim estado_process As Boolean
                estado_process = oSeguridadService.BorrarCentroCosto(CodUsu, dgvCentroCosto.CurrentRow.Cells("CodCentro").Value, Session.sCodUsu, Session.sNomPc, Session.sDirIp)
                If estado_process = True Then
    ...
    Private Sub RowPossesionEquipo(ByVal lista As Janus.Windows.GridEX.GridEX, ByVal codigo As String)
        Try
            Dim rows() As Janus.Windows.GridEX.GridEXRow
            rows = lista.GetRows
            For Each row In rows
                If CInt(row.Cells("SeriePlaca").Value) = codigo Then
                    lista.Row = row.Position
                    lista.Col = 1
                    Exit For
                End If
    ...
    Private Sub miNuevoRubRecurso_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles miNuevoRubRecurso.Click
        Try
            Dim frm As New frmUsuarioEquipo
            frm.CodUsu = txtCodUsu.Text
            If frm.ShowDialog = System.Windows.Forms.DialogResult.OK Then
                ListarRubrosRecurso()
                RowPossesionRubRecurso(dgvRubroRecurso, frm.serialBoard)
            End If
        Catch ex As Exception
    ...
    Private Sub ListarEmpresas()
        Try
            dtEmpresas = oEmpresaUsuario.Mostrar(CodUsu).Tables(0)
    ...
    Private Sub ListarEquipos()
        Try
            dtEquipos = oSeguridadService.MostrarEquipo(CodUsu).Tables(0)
            dgvEquipos.DataSource = dtEquipos
            EnableOptionsOptions()
        Catch ex As Exception
            MsgBox("ERROR AL LISTAR EQUIPOS: " + ex.Message, MsgBoxStyle.Exclamation)
        End Try
    End Sub


    Private Sub btnAsigEmpresa_Click(sender As Object, e As EventArgs) Handles btnAsigEmpresa.Click, btnIngresarEmpresa.Click
        Try
            Dim frm As New frmUsuarioEmpresa
            frm.CodUsu = txtCodUsu.Text
            If frm.ShowDialog = System.Windows.Forms.DialogResult.OK Then
                ListarEmpresas()
            End If
    ...
    Private Sub btnBorrarEmpresa_Click(sender As Object, e As EventArgs) Handles btnBorrarEmpresa.Click
        Try
            'cmbOpRubRecurso.Visible = False
            If MsgBox("¿Está seguro de ELIMINAR el registro seleccionado?", MsgBoxStyle.YesNo, "Información") = MsgBoxResult.Yes Then
                Dim estado_process As Boolean
                estado_process = oEmpresaUsuario.Borrar(CodUsu, dgvEmpresas.CurrentRow.Cells("CodEmp").Value, Session.sCodUsu, Session.sNomPc, Session.sDirIp)
                If estado_process = True Then
                    dtEmpresas = Nothing
                    ListarEmpresas()
                    MsgBox("Se eliminó el registro correctamente.", MsgBoxStyle.Information)
            ...
            End If
        Catch ex As Exception
            MsgBox("ERROR AL ELIMINAR EMPRESAS: " + ex.Message, MsgBoxStyle.Exclamation)
        End Try
    End Sub

    Private Sub btnAsignar_Click(sender As Object, e As EventArgs) Handles btnAsignarEquipo.Click
        Try
            Dim frm As New frmUsuarioEquipo
            frm.CodUsu = txtCodUsu.Text
            If frm.ShowDialog = System.Windows.Forms.DialogResult.OK Then
            ...
            ListarEquipos()
        Catch ex As Exception
            MsgBox("ERROR AL ASIGNAR EQUIPOS: " + ex.Message, MsgBoxStyle.Exclamation)
        End Try
    End Sub

    Private Sub miNuevoEquipo_Click(sender As Object, e As EventArgs) Handles miNuevoEquipo.Click
        Try
            Dim frm As New frmUsuarioEquipo
            frm.CodUsu = txtCodUsu.Text
            If frm.ShowDialog = System.Windows.Forms.DialogResult.OK Then
    ...
    Private Sub miEliminarEquipo_Click(sender As Object, e As EventArgs) Handles miEliminarEquipo.Click
        Try

            If MsgBox("¿Está seguro de ELIMINAR el registro seleccionado?", MsgBoxStyle.YesNo, "Información") = MsgBoxResult.Yes Then
                Dim estado_process As Boolean
                estado_process = oSeguridadService.BorrarEquipo(CodUsu, dgvEquipos.CurrentRow.Cells("SeriePlaca").Value, Session.sCodUsu, Session.sNomPc, Session.sDirIp)
                If estado_process = True Then
                    dtEquipos = Nothing
                    ListarEquipos()
                    MsgBox("Se eliminó el registro correctamente.", MsgBoxStyle.Information)
            ...
            End If
        Catch ex As Exception
            MsgBox("ERROR AL ELIMINAR EQUIPOS: " + ex.Message, MsgBoxStyle.Exclamation)
        End Try
    End Sub
