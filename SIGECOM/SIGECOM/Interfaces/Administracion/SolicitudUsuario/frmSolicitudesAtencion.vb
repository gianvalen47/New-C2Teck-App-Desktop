...
Public Class frmSolicitudesAtencion
    ...
    Private Sub MostrarGrillas()
        If EstadoAtencion = "VS" Then
            ...
            dtMenuOpcionAsignado = oSolicitudUsuarioService.MostrarPersonal().Tables(0)
            dtMenuOpcionAsignado.Clear()

            dv = oSolicitudUsuarioService.MostrarPersonal().Tables(0).DefaultView
            ...
            If MsgBox("¿Está seguro de Modificar el estado de la Solicitud: " & IdSolicitud & " ?", MsgBoxStyle.YesNo, "Advertencia") = MsgBoxResult.Yes And ValidaCampos() = True Then
                ...
                If EstadoAtencion = "VS" Then
                    ...
                    For i = 0 To dgvOpcionMenuAsignado.Rows.Count - 2
                        estado_process = oSolicitudUsuarioService.AsignarPersonal(IdSolicitud, CInt(dgvOpcionMenuAsignado.Rows(i).Cells("IdPer1").Value), Session.sCodUsu, Session.sNomPc, Session.sDirIp)
                    Next
                End If
