...
Public Class frmDiarioBuscarDocTesoreria
    ...
    Private Sub frmDiarioBuscarDocTesoreria_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        Finalizar()
    End Sub

    Private Sub frmDiarioBuscarDocTesoreria_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Sub frmDiarioBuscarDocTesoreria_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub btnBuscar_Click(sender As Object, e As EventArgs) Handles btnBuscar.Click, txtPeriodo.ValueChanged, txtCodCuenta.TextChanged, cmbTipoDoc.ValueChanged, txtSerieDoc.TextChanged, txtNumDoc.TextChanged, txtProveedor.TextChanged
        listaDatos()
    End Sub
    ...
    Private Sub listaDatos()
        Try
            If state_Search = True Then
                ...
                dtDatos = oContabilidadService.MostrarDocumentosTesoreria(Session.sCodEmp, txtPeriodo.Value, txtCodCuenta.Text, cmbTipoDoc.Value, txtSerieDoc.Text, txtNumDoc.Text, txtProveedor.Text).Tables(0)
                dgvDatos.DataSource = dtDatos

                cIdTesoreria.DataPropertyName = dtDatos.Columns("IdTesoreria").ColumnName
                cIdTesoreriaDet.DataPropertyName = dtDatos.Columns("IdTesoreriaDet").ColumnName
                ...
                'cCodJob.DataPropertyName = dtDatos.Columns("IdTesoreria").ColumnName
        ...
        Try
            If MsgBox("¿Está seguro de INSERTAR el(los) Doc(s) seleccionado(s)?", MsgBoxStyle.YesNo, "Advertencia") = MsgBoxResult.Yes Then
                ...
                For i As Integer = 0 To dtDatos.Rows.Count - 1
                    ...
                    If toBoolean(cellSelecion.Value) = True Then
                        rows = dtTable.NewRow
                        rows(0) = dgvDatos.Rows(i).Cells("cIdTesoreria").Value
                        rows(1) = dgvDatos.Rows(i).Cells("cIdTesoreriaDet").Value    'dgvDatos.Rows(i).Cells("cIdTesoreria").Value
                        ...
                        rows(5) = dgvDatos.Rows(i).Cells("cFecDoc").Value
                        rows(6) = dgvDatos.Rows(i).Cells("cIdDocumento").Value
                        'rows(7) =
                        rows(8) = dgvDatos.Rows(i).Cells("cSerDoc").Value
                        rows(9) = dgvDatos.Rows(i).Cells("cNumDoc").Value
                        rows(10) = dgvDatos.Rows(i).Cells("cIdProveedor").Value
                        rows(11) = dgvDatos.Rows(i).Cells("cIdCliente").Value
                        rows(12) = dgvDatos.Rows(i).Cells("cIdPer").Value
                        rows(13) = dgvDatos.Rows(i).Cells("cDocumento").Value
                        rows(14) = dgvDatos.Rows(i).Cells("cDesProv").Value
                        rows(15) = dgvDatos.Rows(i).Cells("cCodMon").Value
                        rows(16) = dgvDatos.Rows(i).Cells("cMontoDol").Value
                        rows(17) = dgvDatos.Rows(i).Cells("cMontoSol").Value
                        rows(18) = dgvDatos.Rows(i).Cells("cSeleccion").Value
                ...
                Else
                    ...
                    For Each Fila As DataRow In dtTable.Rows
                        ...
                        Job.CodJob = Nothing 'IIf(IsDBNull(Fila.Item("CodJob")), Nothing, Fila.Item("CodJob"))
                        registro.Job = Job
                        ...
                        'dtCentroCosto = oContabilidadDetService.MostrarCentroCosto(Fila.Item("IdTesoreriaDet")).Tables(0)
                        dtCentroCosto = oTesoreriaDetService.MostrarCentroCosto(Fila.Item("IdTesoreriaDet")).Tables(0)
                        ...
                        'oTesoreriaDetService.InsertarCentroCosto(Fila.Item("IdTesoreriaDet"), Fila.Item("IdTesoreria"), dtCentroCosto, Session.sCodUsu, Session.sNomPc, Session.sDirIp)
                        '/////////////////////////////////////////////////
    ...
    Private Sub SendFocus_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtSerieDoc.KeyPress, txtPeriodo.KeyPress, txtNumDoc.KeyPress, txtCodCuenta.KeyPress, cmbTipoDoc.KeyPress
    ...
    Private Sub biInsertar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles miInsertar.Click, biInsertar.Click
        Insertar()
    End Sub
