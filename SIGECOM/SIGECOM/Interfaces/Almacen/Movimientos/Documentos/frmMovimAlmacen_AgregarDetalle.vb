...
Public Class frmMovimAlmacen_AgregarDetalle
    ...
    Private Function ValidaCampos() As Boolean
        Try
            ...
            ElseIf txtPreMer.Text.Trim.Length > 0 And CDbl(txtPreMer.Text.Trim) <= 0 Then
                ...
                txtPreMer.Focus()
                Return False
            ElseIf state_button = False And toNumber(txtCanMer.Value) > toNumber(txtStock.Text) And tioMovimineto = "D" Then
                MsgBox("Cantidad no pueder ser mayor que Stock. ", MsgBoxStyle.Information, "Información")
                txtCanMer.Focus()
                Return False
            ElseIf tioMovimineto = "D" And state_button = False And toNumber(txtStock.Text) <= 0 Then
    ...
    Private Sub Modificar(ByVal registro As MoviAlmacenDetService.MoviAlmacenDet)
        Try
            ...
            estado_process = oMoviAlmacenDetService.Actualizar(registro, Session.sCodUsu)
            type_process = "update"
    ...
    Private Sub txtCanMer_Validating(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtCanMer.Validating
        Try
            'If toNumber(txtCanMer.Value) > txtStock.Text And txtStock.Text = "0" And tioMovimineto = "D" Then
            If state_button Then
                Dim registro As MoviAlmacenDetService.MoviAlmacenDet
                registro = oMoviAlmacenDetService.MostrarPorId(toNumber(IdMovimientoDet))
                Dim CantStock As Integer
                CantStock = oLocacionMercaderiaService.MostrarStock(IdLocacion, txtCodMer.Text) + registro.CanMer
                If state_button And toNumber(txtCanMer.Value) > CantStock And tioMovimineto = "D" And Trim(txtCodMer.Text).Substring(0, 3) <> "AAA" Then
            ...
            End If
            ' End If
            'If toNumber(txtCanMer.Value) > toNumber(txtStock.Text) And tioMovimineto = "D" Then
            '    MsgBox("Cantidad no pueder ser mayor que Stock. ", MsgBoxStyle.Information, "Información")
            '    txtCanMer.Focus()
            'End If
            calculateTotal()
        ...
        Try
            If Len(Trim(txtCodMer.Text)) > 0 And state_button = False Then
                ...
                If oLocacionMercaderiaService.Buscar(IdLocacion, txtCodMer.Text) Then
                    ...
                    txtDescripcion.Text = LocacionMercaderia.Mercaderia.DesMer1
                    txtPreMer.Text = LocacionMercaderia.Mercaderia.DeaMer
                    txtDscMer.Text = 0
                    txtGrupo.Text = LocacionMercaderia.Mercaderia.GrupoMerca.CodGru
                    calculateTotal()
                Else
    ...
    Private Sub btnIngresarMercaderia_Click(sender As Object, e As EventArgs) Handles btnIngresarMercaderia.Click

        Try
            Dim frm1 As New frmMovimientoAlmacen_IngresarMercaderia
            frm1.IdLocacionI = IdLocacion
            'frm.state_button = False
            If frm1.ShowDialog = System.Windows.Forms.DialogResult.OK Then
                txtCodMer.Text = frm1.CodMerI
                txtDescripcion.Text = frm1.DesMerI
                txtPreMer.Text = frm1.PreMerI
            End If
        Catch ex As Exception
