            ...
            If MsgBox("¿Está seguro de despachar la mercaderia?", MsgBoxStyle.YesNo, "Advertencia") = MsgBoxResult.Yes And ValidaCampos() Then
                ...
                ElseIf rbVale.Checked Then
                    ...
                    If ValidaValeAlmacen() Then
                        estado_process = oValeMaterialService.GenerarAtencionJob(cmbAlmacen.Value, IdCliente, cmbIdSerieDoc.Value, txtNumero.Text, txtFecha.Text, NumJob, dtTable, IdPersona, Session.sCodUsu, Session.sNomPc, Session.sDirIp)
                    End If
                ElseIf rbProvisional.Checked Then

                    If ValidaValeAlmacen() Then
                        estado_process = oMoviAlmacen.GenerarAtencionJob(cmbAlmacen.Value, IdCliente, cmbIdSerieDoc.Value, txtNumero.Text, txtFecha.Text, NumJob, dtTable, Session.sCodUsu, Session.sNomPc, Session.sDirIp)
                    End If
...
2:          dgvDatos.Select()
    ...
    Private Sub cmbAlmacen_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbAlmacen.ValueChanged
        listaDatos()
        If rbProvisional.Checked Then
            If cmbAlmacen.Value <> 48 Then
                llenarComboSerieDocProvisional()
            End If
        Else
            If cmbAlmacen.Value = 54 Or cmbAlmacen.Value = 67 Or cmbAlmacen.Value = 58 Then    ' Componentes, Equipos Componentes y Garantía Componentes
                llenarComboSerieDoc()
            End If
        End If
    ...
    Private Sub llenarComboSerieDoc()
        Try
    ...
    Private Sub llenarComboSerieDocProvisional()
        Try
            'dtTipos = oMaestro.MostrarSerieDocumento(54, 3, "").Tables(0)
            dtTipos = oMaestro.MostrarSerieDocumento(cmbAlmacen.Value, 2, "O").Tables(0)
    ...
    Private Sub rbTransInt_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbTransInt.CheckedChanged
        ...
        If rbTransInt.Checked Then
            cmbAlmacen.Value = 3                                                  'Cambio pedido por chipa para que seleccione servicio y asi sugerir el numero
            txtNumero.Text = oTransferenciaService.SugerirNumero(cmbAlmacen.Value)
            cmbIdSerieDoc.Enabled = False
            cmbIdSerieDoc.SelectedIndex = -1
            IdPersona = 0
    ...
    Private Sub rbProvisional_CheckedChanged(sender As Object, e As EventArgs) Handles rbProvisional.CheckedChanged
