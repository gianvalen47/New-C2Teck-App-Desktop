...
Public Class frmVale
    ...
    Private oPrecio As New PrecioService.PrecioServiceClient
    ...
    Private Sub Initialize_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Else                    'Nuevo
            ...
            Me.Size = New System.Drawing.Size(781, 220)
    ...
    Private Sub Eliminar()
        Try
            Dim estado_process As Boolean
            estado_process = oValeMaterialService.Borrar(IdVale, Session.sCodUsu, Session.sNomPc, Session.sDirIp)
            type_process = "delete"
            If estado_process = True Then
                Me.DialogResult = System.Windows.Forms.DialogResult.OK
            Else
    ...
    Private Sub llenarComboTipos()
        ...
        cmbIdSerieDoc.Value = IdSerieDoc
        dtTipos = Nothing
    End Sub
    ...
    Private Sub eliminarDetalle()
        Try
            ...
            If MsgBox("¿Está seguro de ELIMINAR el registro con CÓDIGO = " + dgvDatos.CurrentRow.Cells("CodMer").Text.ToString + " ?", MsgBoxStyle.YesNo, "Información") = MsgBoxResult.Yes Then
                ...
                estado_process = oValeMaterialDetService.Borrar(toNumber(dgvDatos.CurrentRow.Cells("IdValeDet").Text), IdVale, Session.sCodUsu, Session.sNomPc, Session.sDirIp)
                If estado_process = True Then
                    dtDatos = Nothing
                    listaDatos()
            ...
            If MsgBox("¿Está seguro de IMPORTAR los datos?", MsgBoxStyle.YesNo, "Advertencia") = MsgBoxResult.Yes Then
                ...
                If dgvImportarExcel.RowCount > 0 Then
                    For Each fila As DataGridViewRow In dgvImportarExcel.Rows
                        ...
                        Dim valeMaterialcab As New ValeMaterialService.ValeMaterial
                        valeMaterialcab = oValeMaterialService.MostrarPorId(IdVale)

                        codigo = toBlank(fila.Cells("Codigo").Value)
                        cantidad = toBlank(fila.Cells("Cantidad").Value)

                        registro.IdValeDet = 0
                        valeMaterial.IdVale = IdVale
                        registro.ValeMaterial = valeMaterial
                        mercaderia.CodMer = toNull(codigo)
                        registro.Mercaderia = mercaderia
                        registro.CanMer = toNumber(cantidad)
                        registro.PreMer = toDouble(oPrecio.PrecioVenta(IdLocacion, valeMaterialcab.Cliente.IdCliente, codigo, valeMaterialcab.Moneda.CodMon, valeMaterialcab.FecDoc)) 'LocacionMercaderia.Mercaderia.CosDol  'LocacionMercaderia.Mercaderia.DeaMer
                        registro.DscMer = toDouble(oPrecio.DescuentoVenta(IdLocacion, valeMaterialcab.Cliente.IdCliente, codigo))
