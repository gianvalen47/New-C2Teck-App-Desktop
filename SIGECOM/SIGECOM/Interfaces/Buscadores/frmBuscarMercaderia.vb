...
Public Class frmBuscarMercaderia
  ...
  Private Sub RowPossesion(ByVal lista As Janus.Windows.GridEX.GridEX, ByVal tabla As DataTable, ByVal nombreCampo As String, ByVal codigo As String)
    ...
    Private Sub frmBuscarMercaderia_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
  ...
  Private Sub Seleccionar()
    Try
      codigo = dgvDatos.CurrentRow.Cells("CodMer").Text
            ...
            If ConsultaFormulario = "pedidointerno" Then
                descripcion = dgvDatos.CurrentRow.Cells("DesMer2").Text
            Else
                descripcion = dgvDatos.CurrentRow.Cells("DesMer1").Text
            End If
  ...
  Private Sub listaDatos()
    Try
      If state_Search = True Then
                ...
                'Dim clase As New MercaderiaService.ClaseMerca
                'Dim rubro As New MercaderiaService.Rubro

                'registro.CodMer = toBlank(txtCodMer.Text)
                'registro.DesMer1 = toBlank(txtDesMer1.Text)
                'clase.IdClase = toBlank(cmbIdClase.Value)
                '        registro.ClaseMerca = clase
                '        rubro.CodRub = toBlank(cmbCodRub.Value)
                '        registro.Rubro = rubro
                dtDatos = oMercaderiaService.Filtrar(toBlank(txtCodMer.Text), Session.sCodEmp, toBlank(txtDesMer1.Text), toBlank(cmbCodRub.Value), "", "", toBlank(cmbIdClase.Value)).Tables(0)
