...
Public Class frmListaPrecioNuevo_AprobarGerencia
    ...
    Private Sub frmListaPrecioNuevo_AprobarGerencia_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Try
    ...
    Private Sub frmListaPrecioNuevo_AprobarGerencia_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub frmListaPrecioNuevo_AprobarGerencia_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.Text = "Aprobar la Lista de precio N°:" & IdLista
        cbAprobar.Checked = True
        btnAprobar.Focus() '------- Agregado el 30/05/2012 -------
        Me.Size = New System.Drawing.Size(876, 484)
    ...
    Private Sub ListarDetallesPendientes()
        ...
        Try
            dtDetallesPendientes = oListaPrecioCabService.MostrarPendientes(IdLista).Tables(0)
            'AgregandoMargen()
    ...
    '    dtCopia.Columns.Add(New DataColumn("Margen", Type.GetType("System.String")))

    '    dtCopia.Rows.Add(New Object() {"1", "1", "Nuevo", "Nuevo", "Nuevo", "Nuevo", "5.00", "Nuevo", "Nuevo", "Nuevo", "0.00", "true", "0"})

    '    dtDetalles = dtCopia.Copy
    '    dtDetalles.Clear()

    '    'Dim rows() As Janus.Windows.GridEX.GridEXRow
    '    'rows = dgvPendientes.GetRows()

    '    For Each row In dtDetallesPendientes
    ...
    '        rowDataN(10) = Convert.ToDouble(row.Cells("Costo").Text)
    '        rowDataN(11) = True
    '        rowDataN(12) = (Convert.ToDouble(row.Cells("Costo").Text) / Convert.ToDouble(row.Cells("Precio").Text)) * 100

    '        dtDetalles.Rows.Add(rowDataN)
    '    Next

    '    dgvPendientes.DataSource = dtDetalles
            ...
            If cbAprobar.Checked Then
                ...
                If rows.Count <> 0 Then
                    ...
                    If MsgBox("¿Está seguro de APROBAR la Lista de precio N°: " & IdLista & " ?", MsgBoxStyle.YesNo, "Advertencia") = MsgBoxResult.Yes Then
                        ...
                        estado_process = oListaPrecioCabService.AprobacionGerencia(IdLista, dtDetalles, txtObservacion.Text, Session.sCodUsu, Session.sNomPc, Session.sDirIp)

                        If estado_process Then
                            MsgBox("Se aprobo la lista de precios correctamente", MsgBoxStyle.Information)
            ...
            Else
                ...
                Else
                    If MsgBox("¿Está seguro de RECHAZAR la Lista de precio N°: " & IdLista & " ?", MsgBoxStyle.YesNo, "Advertencia") = MsgBoxResult.Yes Then
                        ...
                        estado_processrec = oListaPrecioCabService.Rechazar(IdLista, txtObservacion.Text, Session.sCodUsu, Session.sNomPc, Session.sDirIp)
