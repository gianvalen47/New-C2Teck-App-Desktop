...
Public Class frmListaOfertaNuevo_Nuevo

    Private oListaOfertaCabService As New ListaOfertaCabService.ListaOfertaCabServiceClient
    Private oListaOfertaDetService As New ListaOfertaDetService.ListaOfertaDetServiceClient
    ...
    Private Sub frmListaOfertaNuevo_Nuevo_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If state_button Then    'Modificar 
            ...
            listaCorreos()
            If iEstado <> 1 Then
                Me.Size = New System.Drawing.Size(993, 527)
                gbCorreos.Visible = False
            Else
                Me.Size = New System.Drawing.Size(993, 686)
                gbCorreos.Visible = True
            End If
            Me.Text = "LISTA OFERTA Nº " + Chr(34) + txtLista.Text.ToString + Chr(34)
            'dgvDatos.Select()
        ...
        Else                          'Nuevo
            ...
            Me.Size = New System.Drawing.Size(993, 268)

            gbEstado.Visible = False
            gbDetalle.Visible = False
            'gbCorreos.Visible = False
            Me.Text = "Registrar nueva Lista de Oferta"
            activar()
    ...
    Private Sub frmListaOfertaNuevo_Nuevo_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub frmListaOfertaNuevo_Nuevo_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Try
            oListaOfertaCabService.Close()
            oListaOfertaDetService.Close()
        ...
        Try
            ...
            If dgvDatos.RowCount > 0 Then
                If IsDBNull(dgvDatos.CurrentRow.Cells(0).Text) = False Then
                    codigo = dgvDatos.CurrentRow.Cells("IdOfertaDet").Text
                End If
            End If
            dtDatos = Nothing
            listaDatos()
    ...
    Private Sub RowPossesion(ByVal lista As Janus.Windows.GridEX.GridEX, ByVal codigo As String)
        Try
            ...
            For Each row In rows
                If CInt(row.Cells("IdOfertaDet").Value) = codigo Then
    ...
    Private Sub listaDatos()
        Try
            dtDatos = oListaOfertaDetService.Mostrar(IdOferta).Tables(0)
    ...
    Private Sub ObtenerRegistro()
        Try
            Dim registro As ListaOfertaCabService.ListaOfertaCab
            registro = oListaOfertaCabService.Obtener(IdOferta)

            IdOferta = registro.IdOferta
            iEstado = registro.EstadoListaPrecio.IdEstado

            txtLista.Text = registro.IdOferta
            txtNombreLista.Text = registro.Nombre
            cmbMoneda.Value = registro.Moneda.CodMon
            txtObservacion.Text = registro.Observacion
            lblEstado.Text = registro.EstadoListaPrecio.DesEstado

            If IsDBNull(registro.FecVencimiento) Then
    ...
    Private Sub enableOpciones()
        ...
        biAprobar.Enabled = IIf(iEstado = 2 Or iEstado = 3 Or (oListaOfertaCabService.BuscarAprobacionPendiente(IdOferta) And iEstado = 4), True, False)
    ...
    Private Sub biGuardar_Click(sender As Object, e As EventArgs) Handles biGuardar.Click
        Try
            ...
            And ValidaCampos() Then
                ...
                Dim registro As New ListaOfertaCabService.ListaOfertaCab
                Dim Moneda As New ListaOfertaCabService.Moneda
                Dim empresa As New ListaOfertaCabService.Empresa
                Dim estado As New ListaOfertaCabService.EstadoListaPrecio

                registro.IdOferta = IdOferta
                ...
                Else                                  'Nuevo
                    estado.IdEstado = 1
                    registro.EstadoListaPrecio = estado
                    Insertar(registro)
                End If
            End If
        Catch ex As Exception
            MsgBox("ERROR AL GUARDAR LA CABECERA DE LA LISTA DE OFERTA: " + ex.Message, MsgBoxStyle.Exclamation)
        End Try
    End Sub
    ...
    Private Sub Insertar(ByVal registro As ListaOfertaCabService.ListaOfertaCab)
        Try
            Dim estado_process As Integer
            estado_process = oListaOfertaCabService.Insertar(registro)
            type_process = "insert"
            If estado_process > 0 Then
                IdOferta = estado_process
    ...
    Private Sub Modificar(ByVal registro As ListaOfertaCabService.ListaOfertaCab)
        Try
            Dim estado_process As Boolean
            estado_process = oListaOfertaCabService.Actualizar(registro)
            type_process = "update"
    ...
    Private Sub eliminarDetalle()
        Try
            ...
            If MsgBox("¿Está seguro de ELIMINAR el registro con CÓDIGO = " + dgvDatos.CurrentRow.Cells("CodMer").Text.ToString + " ?", MsgBoxStyle.YesNo, "Información") = MsgBoxResult.Yes Then
                ...
                Dim estado_process As Boolean
                estado_process = oListaOfertaDetService.Borrar(dgvDatos.CurrentRow.Cells("IdOfertaDet").Value, Session.sCodUsu, Session.sNomPc, Session.sDirIp)
                If estado_process = True Then
                    ObtenerRegistro()
                    dtDatos = Nothing
                    listaDatos()
    ...
    Private Sub NuevoDetalle()
        Try
            ...
            While lLog
                Dim frm As New frmListaOfertaNuevo_Detalle
                frm.state_button = False
                frm.IdOferta = IdOferta
                'frm.estado = 1
                ...
                If frm.ShowDialog = System.Windows.Forms.DialogResult.OK Then
                    ...
                    If iEstado = 1 Then
                        ...
                        gbCorreos.Visible = True
                        RowPossesion(dgvDatos, frm.IdOfertaDet)
                    End If
                Else
                    lLog = False
                End If
    ...
    Private Sub btnEnviar_Click(sender As Object, e As EventArgs) Handles btnEnviar.Click
        Try
            If dgvDatos.RowCount > 0 Then
                If MsgBox("¿Está seguro de ENVIAR la Lista de Oferta Nº" & txtLista.Text & " ?", MsgBoxStyle.YesNo, "Advertencia") = MsgBoxResult.Yes Then
                    ...
                    If rows.Count <> 0 Then
                        ...
                        Dim estado_process As Boolean
                        estado_process = oListaOfertaCabService.Enviar(IdOferta, Cadena, Session.sCodUsu, Session.sNomPc, Session.sDirIp)

                        If estado_process Then
                            MsgBox("Se envió correctamente la Lista de Ofertas", MsgBoxStyle.Information)
                            ObtenerRegistro()
                            enableOpciones()
                            actualizarDetalles()
                            Me.Size = New System.Drawing.Size(985, 521)
                            gbCorreos.Visible = False
                        Else
            ...
            End If
        Catch ex As Exception
            MsgBox("Error al Enviar la Lista de Ofertas: " + ex.Message, MsgBoxStyle.Exclamation)
        End Try
    End Sub
    ...
    Private Sub CreacionTable()
        Try
            ...
            dtCopia.Rows.Add(New Object() {"Codigo", "0.00", "Obs"})
    ...
    '        'estado_process = oListaPrecioDetService.ValidarPrecios(dtInsertarMasivo)

    '        If estado_process = "" Then
    '            InsertarMasivo()
    '        Else
    '            If MsgBox(estado_process & Environment.NewLine & Environment.NewLine &
    ...
    Private Sub InsertarMasivo()
        ...
        Try
            Dim estado_process As Boolean
            estado_process = oListaOfertaDetService.InsertarMasivo(IdOferta, dtInsertarMasivo, Session.sCodUsu, Session.sNomPc, Session.sDirIp)

            If estado_process = True Then
                MsgBox("Se Inserto la Lista de Oferta correctamente", MsgBoxStyle.Information)
                listaDatos()
            Else
                MsgBox("Error en el proceso, comuníquese con el departamento de sistemas...!", MsgBoxStyle.Critical)
            End If
        Catch ex As Exception
            MsgBox("ERROR AL INSERTAR LA LISTA DE OFERTA : " + ex.Message, MsgBoxStyle.Exclamation)
            actualizarDetalles()
        End Try
    ...
    Private Sub mostrarDetalle()
        Try
            ...
            If cmOpciones.Enabled = True Then
                ...
                frm.IdOfertaDet = dgvDatos.CurrentRow.Cells("IdOfertaDet").Text
                frm.IdOferta = IdOferta
    ...
    Private Sub biAprobar_Click(sender As Object, e As EventArgs) Handles biAprobar.Click
        Try
            If txtLista.Text <> "" Then
                ...
                If iEstado = 2 Then
                    frm.IdOferta = toNumber(txtLista.Text)
                    ...
                    End If
                Else
                    frmGer.IdOferta = toNumber(txtLista.Text)
