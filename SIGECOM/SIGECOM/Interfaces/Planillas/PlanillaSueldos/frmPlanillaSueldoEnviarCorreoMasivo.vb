...
Public Class frmPlanillaSueldoEnviarCorreoMasivo
    ...
    Private Sub CrearCarpeta()
        Try
            ...
            If Not Directory.Exists("D:\BoletaSueldoElectronicas\" & Session.sDesEmp & "\" & idPlanilla.ToString()) Then
                Directory.CreateDirectory("D:\BoletaSueldoElectronicas\" & Session.sDesEmp & "\" & idPlanilla.ToString())
            End If
    ...
    Private Sub GenerarPDF()
        ...
        dtDetalle = oPlanillaDet.Mostrar(idPlanilla).Tables(0)
        ...
        Else
            For Each Fila As DataRow In dtDetalle.Rows
                ...
                If registro.TipoPlanilla.IdTipo = 8 Then 'CTS
                    dtReporte = oPlanilla.ImprimirBoletaPago(Session.sCodEmp, idPlanilla, Fila.Item("IdPer"), 0, "", 0, 8).Tables(0)
                    If Session.sCodEmp = "05" Then
                        reporteCTSEquimap.SetDataSource(dtReporte)
                        ExportToPDF(reporteCTSEquimap, Fila.Item("IdPer"))
                    ElseIf Session.sCodEmp = "02" Then
                        reporteCTSAmazonica.SetDataSource(dtReporte)
                        ExportToPDF(reporteCTSAmazonica, Fila.Item("IdPer"))
                    Else
                        reporteCTS.SetDataSource(dtReporte)
                        ExportToPDF(reporteCTS, Fila.Item("IdPer"))
                    End If
                Else
                    dtReporte = oPlanilla.ImprimirBoletaPago(Session.sCodEmp, idPlanilla, Fila.Item("IdPer"), 0, "", 0, 1).Tables(0)
                    dtSubreporte = oPlanilla.ImprimirBoletaPago(Session.sCodEmp, idPlanilla, Fila.Item("IdPer"), 0, "", 0, 1).Tables(0)
    ...
    Public Function ExportToPDF(rpt As ReportDocument, codigo As String) As String
        ...
        Try
            ...
            diskOpts.DiskFileName = "D:\BoletaSueldoElectronicas\" & Session.sDesEmp & "\" & idPlanilla.ToString() & "\" & periodo & mes & codigo & ".pdf"
    ...
    Private Sub ListarArchivos()
        Try
            Dim d As New DirectoryInfo("D:\BoletaSueldoElectronicas\" & Session.sDesEmp & "\" & idPlanilla)
            ...
            For Each f As FileInfo In d.GetFiles
                idper2 = Mid(f.Name, 7, Len(f.Name) - (Len(f.Name) - 7))
    ...
    Private Sub EnviarCorreo(destino As String, ruta As String, nombre As String)
        ...
        Try
            ...
            If Session.sCodEmp = "02" Or Session.sCodEmp = "05" Then
                smtp.EnableSsl = True
            End If
            smtp.Port = 587
